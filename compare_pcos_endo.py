...
def load_data(root: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    pcos_path = root / "(Main_Dataset)_PCOS_data_without_infertility.xlsx"
    endo_path = root / "(Supplementary_Dataset)_structured_endometriosis_data (1).xlsx"
    pcos = pd.read_excel(pcos_path, sheet_name="Full_new")
    endo = pd.read_excel(endo_path, sheet_name="in")
    return pcos, endo
...
def main():
    root = Path(r"C:\Users\uik07687\OneDrive - Aumovio SE\Desktop\hack")
    pcos, endo = load_data(root)
    ...
    save_outputs(root, combined, diffs, metrics, coef_df)
